...
# WebGLM-2B(bing): python web_demo.py -w THUDM/WebGLM-2B --searcher bing --retriever_ckpt_path ./download/retriever-pretrained-checkpoint
# WebGLM-10B(bing): python web_demo.py --searcher bing --retriever_ckpt_path ./download/retriever-pretrained-checkpoint
# WebGLM-2B(google): python web_demo.py -w THUDM/WebGLM-2B --retriever_ckpt_path ./download/retriever-pretrained-checkpoint
# WebGLM-10B(google): python web_demo.py --retriever_ckpt_path ./download/retriever-pretrained-checkpoint
...
# 2.使用非全局模式
# 3.显存不够（RTX 3060 Laptop，只有6G），要12G或者24G的显卡。建议能装2~4张24G显存显卡的机箱。
